...
using ECommerce.Core.ComponentModel;
...
{
    /// <summary>
    ///插件管理 Sets the application up for the plugin referencing
    /// </summary>
    public class PluginManager
    {
        ...
        public static void Initialize()
        {
            using (new WriteLockDisposable(Locker))
            {
                ...
                //应用程序启动发生时，添加详细异常处理，以防止应用程序一起启动
                ...
                try
                {
                    ...
                    //获取bin目录下所有文件。get list of all files in bin
                    ...
                    foreach (var dfd in GetDescriptionFilesAndDescriptors(pluginFolder))
                    {
                        ...
                        // 验证插件版本是否支持本版本系统。ensure that version of plugin is valid
                        if (!pluginDescriptor.SupportedVersions.Contains(EVersion.CurrentVersion, StringComparer.InvariantCultureIgnoreCase))
                        {
        ...
        private static IEnumerable<KeyValuePair<FileInfo, PluginDescriptor>> GetDescriptionFilesAndDescriptors(DirectoryInfo pluginFolder)
        {
            ...
            //创建列表 (<file info, parsed plugin descritor>)
            var result = new List<KeyValuePair<FileInfo, PluginDescriptor>>();
            //添加显示顺序与途径到列表。add display order and path to list
        ...
        /// <summary>
        /// 检查是否存在Plugins文件夹（满足Plugins存在上级目录）
        /// </summary>
